...
def cust_query(db1,db2,fileDir,file_name,queryy):
    ...
    df_q_db = pd.DataFrame(columns=['Test_CaseId', 'Test_Type', 'Source_TableName', 'Target_TableName', 'Status'])
    for q in range(0,NR):
        print (q)
            ...
            if squery == 'None' or sname == 'None':
                ...
                try:
                    ...
                    if df.shape[0]==0:
                        ...
                        row_db = [tid,'Custom Query Check', squery,tquery, 'Success']
                    else:
                        row = [tid,'Custom Query Check',squery,tquery, 'Fail']
                        row_db = [tid,'Custom Query Check', squery,tquery, 'Fail']
                        zip_list.append(tid)
                        fileDir = os.path.dirname(os.path.realpath('__file__'))
                        print(fileDir)
