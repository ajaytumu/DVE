...
def special(db1,db2, fileDir,spe):
    ...
    df_special_db=pd.DataFrame(columns=['Test_CaseId', 'Test_Type', 'Source_TableName', 'Target_TableName', 'Status'])
    try:
        for x in range(0, NR):
            t = x + 2
            print('tid', t)
                    ...
                    try:
                        try:
                            ...
                            if char.shape[0] == df.shape[0]:
                                ...
                                df_special.loc[x] = add_rows
                                add_rows_db=[tid,t_type,tablename,columnname,'Success']
                                df_special_db.loc[x]=add_rows_db
                                df_special.to_excel(fileDir + '\\' + 'Report for Special_Char_Check' + '.xlsx',
                                                    index=False)
                            else:
                                add_rows = [tid, t_type, databasename, tablename, columnname, special, 'Fail']
                                df_special.loc[x] = add_rows
                                add_rows_db=[tid,t_type,tablename,columnname,'Fail']
                                df_special_db.loc[x]=add_rows_db
                                df_special.to_excel(fileDir + '\\' + 'Report for Special_Char_Check' + '.xlsx',
                                                    index=False)
                        except:
        ...
        return render_template("home.html")
    print("Time: ", end - start)
    return df_special,df_special_db
