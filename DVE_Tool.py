...
engine = create_engine(URL(
    account = 'th73882.central-india.azure',
    user = 'AJAY1325',
    password = 'Ajay@1325',
    database = 'result_db',
    schema = 'dbo',
...
esheets=[df1,df2,df3,df4,df5,df6,df7,df8]
    ...
    if request.method == 'POST':
        ...
        if request.form.get("struct"):
            try:
                ...
                if v==True:
                    ...
                    df_sv = structure(db1, db2, fileDir,structt)
                print(df_sv)
                df_sv['TimeStamp'] = dt_string
                df_sv.to_sql('final_table', con=engine,if_exists = 'append', index=False)
                data_struct = df_sv[:3].to_html(classes='mystyle', index=False)
        ...
        if request.form.get("Special"):
            ...
            if v == True:
                ...
                df_special,df_special_db= special(db1,db2, fileDir,spe)
            data_special= df_special[:3].to_html(classes='mystyle', index=False)
            df_special_db['TimeStamp'] = dt_string
            df_special_db.to_sql('final_table', con=engine,if_exists = 'append', index=False)
        ...
        if request.form.get("length check"):
            ...
            try:
                data= esheets[7]
                print(data)
                df_length,df_length_db = datatype(db1,db2, fileDir,data, datanamedb1, datanamedb2)
                print(df_length)
                df_length_db['TimeStamp'] = dt_string
                df_length_db.to_sql('final_table', con=engine,if_exists = 'append', index=False)
        ...
        if request.form.get("Date format check"):
            ...
            df_date_db = pd.DataFrame(columns=['Test_CaseId', 'Test_Type', 'Source_TableName', 'Target_TableName', 'Status'])
            try:
                for d in range(0,NR):
                    t = d + 2
                    print('tid',t)
                            ...
                            try:
                                try:
                                    ...
                                    if df.iloc[0][0]== 'Date'.lower() or df.iloc[0][0] == "Date":
                                        ...
                                        add_rows_db = [tid,'Date_Check',tablename, columnname, 'Success']
                                        df_date.loc[d] = add_rows
                                        df_date_db.loc[d] = add_rows_db
                                        ...
                                        df_date.loc[d] = add_rows
                                        add_rows_db = [tid,'Date_Check',tablename, columnname, 'Fail']
                                        df_date_db.loc[d] = add_rows_db
                                        df_date.to_excel(fileDir +'\\'+'Report for Date_Check'+'.xlsx', index = False)
                                except:
                                    query = "SELECT data_type FROM information_schema.columns WHERE table_catalog = '" + username + "' and table_schema = '" + databasename + "' and table_name = '" + tablename + "' and column_name = '" + columnname + "'"
            ...
            df_date_db['TimeStamp'] = dt_string
            df_date_db.to_sql('final_table', con=engine,if_exists = 'append', index=False)
        ...
        if request.form.get("query check"):
            try:
                queryy = esheets[3]
                df_q,df_q_db = cust_query(db1,db2,fileDir,file_name,queryy)
                df_q_db['TimeStamp'] = dt_string
                df_q_db.to_sql('final_table', con=engine,if_exists = 'append', index=False)
                if df_q.shape[0]!=0:
        ...
        if request.form.get("default"):
                ...
                if v==True:
                    ...
                    df_default,df_default_db= default(db1, db2, fileDir,defaultt)
                df_default_db['TimeStamp'] = dt_string
                df_default_db.to_sql('final_table', con=engine,if_exists = 'append', index=False)
        ...
        if request.form.get("full comparison"):
            ...
            print(df_full)
            df_full['TimeStamp'] = dt_string
            df_full.to_sql('final_table', con=engine,if_exists = 'append', index=False)
        ...
        if request.form.get("Max"):
            try:
                ...
                df_max['TimeStamp'] = dt_string
                df_max.to_sql('final_table', con=engine, if_exists = 'append', index=False) #make sure index is False, Snowflake doesnt accept indexe
        ...
        if request.form.get("Min"):
            try:
                ...
                df_min['TimeStamp'] = dt_string
                df_min.to_sql('final_table', con=engine,if_exists = 'append', index=False) 
        ...
        if request.form.get("Mean"):
            ...
            df_mean['TimeStamp'] = dt_string
            df_mean.to_sql('final_table', con=engine, if_exists = 'append', index=False)
        ...
        if request.form.get("Std"):
            ...
            df_std['TimeStamp'] = dt_string
            df_std.to_sql('final_table', con=engine, if_exists = 'append', index=False)
        ...
        if request.form.get("Sum"):
            try:
                ...
                df_sum['TimeStamp'] = dt_string
                df_sum.to_sql('final_table', con=engine, if_exists = 'append', index=False)
        ...
        if request.form.get("Statistics"):
            try:
                ...
                df_stat['TimeStamp'] = dt_string
                data_stats=df_stat[:3].to_html(classes='mystyle', index=False)
                df_stat.to_sql('final_table', con=engine, if_exists = 'append', index=False)
        ...
        if request.form.get("count"):
            try:
                ...
                if v == True:
                    ...
                    print(df_count)
                df_count['TimeStamp'] = dt_string
                df_count.to_sql('final_table', con=engine,if_exists = 'append', index=False) 
        ...
        if request.form.get("duplicate"):
            try:
                ...
                if v == True :
                    ...
                    df_duplicate,df_duplicate_db = duplicate(db1, db2, fileDir,dupp)
                df_duplicate_db['TimeStamp'] = dt_string
                Report_for_Duplicate_Check = df_duplicate[:3].to_html(classes='mystyle', index=False)
                df_duplicate_db.to_sql('final_table', con=engine, if_exists = 'append', index=False)
        ...
        if request.form.get("Null check"):
            try:
                ...
                if v==True:
                    ...
                    df_null,df_null_db = null(db1, db2, fileDir,nulll)
                df_null_db['TimeStamp'] = dt_string
                df_null_db.to_sql('final_table', con=engine, if_exists = 'append', index=False)
